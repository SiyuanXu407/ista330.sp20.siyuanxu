...
function saveLabel(name, x, y, number, imageId) {
   //TODO: insert a new record in the word table and return its id
	return postgrePool.query('insert into word(name, x, y, number, imageid) values($1,$2,$3,$4,$5) returning id',[name, x, y, number, imageId])
	.then(result=>result.rows[0].id);
}

function getAllImages() {
	  return postgrePool.query('select * from image')
	  .then(result => result.rows);
	}

function getAllWord() {
	  return postgrePool.query('select * from word')
	  .then(result => result.rows);
	}

module.exports = { saveTheme, getAllThemes, getImageIds, getImageName, saveImage, saveLabel, getLabel, getLabels , updateLabel, getAllImages, getAllWord}
